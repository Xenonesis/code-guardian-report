...
if [ "$BRANCH" != "main" ] && [ "$BRANCH" != "master" ]; then
  echo "Not on main/master branch, skipping contributor update"
  exit 0
fi
...
if echo "$LAST_COMMIT_MSG" | grep -q "Auto-update contributors"; then
  echo "Last commit was already a contributor update, skipping"
  exit 0
fi
...
if git diff --cached --name-only | grep -q "README.md"; then
  ...
  if echo "$STAGED_CONTENT" | grep -q "Community & Contributors"; then
    echo "README.md contributors section is being updated, skipping check"
    exit 0
  fi
fi
