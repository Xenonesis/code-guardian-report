...
config.requiredLanguages.forEach(lang => {
  const langRulePattern = new RegExp(`languages:\\s*\\[([^\\]])*'${lang}'`, 'g');
  const langRules = [...analyzerContent.matchAll(langRulePattern)];
  
  if (langRules.length < config.minRulesPerLanguage) {
    console.warn(`  ⚠️  ${lang} has only ${langRules.length} rules (minimum: ${config.minRulesPerLanguage})`);
  } else {
...
if (fs.existsSync(displayFile)) {
  ...
  if (languageCount < config.requiredLanguages.length) {
    console.warn(`  ⚠️  UI might not show all languages (found ${languageCount}, expected ${config.requiredLanguages.length})`);
  }
} else {
...
console.log('\n' + '='.repeat(60));
if (allChecksPassed) {
  console.log('✅ All checks passed! Multi-language support is properly configured.\n');
  process.exit(0);
} else {
