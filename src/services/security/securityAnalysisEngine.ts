...
export function calculateSecurityScore(issues: SecurityIssue[]): number {
  ...
  const severityDistributionPenalty =
    calculateSeverityDistributionPenalty(issues);
...
function calculateSeverityDistributionPenalty(issues: SecurityIssue[]): number {
  ...
  const criticalRatio =
    issues.filter((i) => i.severity === "Critical").length / totalIssues;
  const highRatio =
    issues.filter((i) => i.severity === "High").length / totalIssues;
...
function applySecurityModifiers(
  baseScore: number,
  ...
  const hasInjectionIssues = issues.some(
    (issue) =>
  ...
  const hasSecretIssues = issues.some(
