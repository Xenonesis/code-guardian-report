...
export class FrameworkDetectionEngine {
    ...
    return {
      ...
      confidence:
        uniqueFrameworks.length > 0
          ? Math.max(...uniqueFrameworks.map((f) => f.confidence))
          : 0,
      detectionMethod: this.determineDetectionMethod(
        uniqueFrameworks,
        dependencies
      ),
    };
  ...
  private calculateFrameworkConfidence(
    ...
    const contentMatches = pattern.contentPatterns.filter((regex) =>
      regex.test(content)
    ).length;
    ...
    if (dependencies && pattern.dependencies.length > 0) {
      const depMatches = this.matchDependencies(
    ...
    const configMatches = this.matchFilePatterns(
      pattern.configFiles,
      filenames
    );
    confidence +=
      (configMatches / Math.max(1, pattern.configFiles.length)) * 10;

    // Directory structure matching (10% weight)
    const structureMatches = this.matchDirectoryStructure(
      pattern.directoryStructure,
      filenames
    );
    confidence +=
      (structureMatches / Math.max(1, pattern.directoryStructure.length)) * 10;
  ...
  private matchDependencies(
    requiredDeps: string[],
    actualDeps: DependencyInfo[]
  ): number {
    let matches = 0;
  ...
  private matchDirectoryStructure(
    ...
    for (const structure of requiredStructure) {
      if (
        filenames.some((filename) => filename.includes(structure.toLowerCase()))
      ) {
        matches++;
      }
    }
