    ...
    const models: AIModel[] = data.data
      ...
      .map((model: any) => {
        ...
        if (model.id.includes('gpt-4o')) {
          ...
          description = model.id.includes('32k') ? 'GPT-4 with extended 32k context' : 'Most capable GPT-4 model';
 ...
 */
export async function fetchClaudeModels(apiKey: string): Promise<ModelDiscoveryResult> {
  try {
 ...
 */
export async function fetchGroqModels(apiKey: string): Promise<ModelDiscoveryResult> {
  try {
 ...
 */
export async function discoverModels(provider: string, apiKey: string): Promise<ModelDiscoveryResult> {
  if (!apiKey) {
