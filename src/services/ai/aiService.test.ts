...
describe("AIService", () => {
  ...
  describe("Rate Limiting", () => {
    it("should allow requests within rate limit", async () => {
      ...
      const checkRateLimit = (
        aiService as unknown as { checkRateLimit: (id: string) => void }
      ).checkRateLimit.bind(aiService);
  ...
  describe("API Key Management", () => {
    it("should return empty array when no keys are stored", async () => {
      ...
      const getStoredAPIKeysAsync = (
        aiService as unknown as {
          getStoredAPIKeysAsync: () => Promise<
            Array<{ id: string; name: string; apiKey: string; model?: string }>
          >;
        }
