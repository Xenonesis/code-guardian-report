...
Provide specific, actionable insights based on the actual findings, not generic advice. Use professional security terminology while remaining accessible to development teams.`
    ...
    const criticalIssues = analysisResults.issues.filter((i: SecurityIssue) => i.severity === 'Critical');
    const highIssues = analysisResults.issues.filter((i: SecurityIssue) => i.severity === 'High');
    const owaspCategories = [...new Set(analysisResults.issues.map((i: SecurityIssue) => i.owaspCategory).filter(Boolean))];
    const avgCvssScore = analysisResults.issues
      .filter((i: SecurityIssue) => i.cvssScore)
      .reduce((sum: number, i: SecurityIssue) => sum + (i.cvssScore || 0), 0) /
      analysisResults.issues.filter((i: SecurityIssue) => i.cvssScore).length || 0;
...
${relatedIssues.slice(0, 5).map((issue: SecurityIssue) =>
...
Prioritize based on CVSS scores, business impact, and implementation effort.`
    ...
    const criticalHighIssues = analysisResults.issues.filter((i: SecurityIssue) =>
      i.severity === 'Critical' || i.severity === 'High'
    );

    const quickWins = analysisResults.issues.filter((i: SecurityIssue) =>
...
${criticalHighIssues.slice(0, 8).map((issue: SecurityIssue) =>
...
${quickWins.slice(0, 5).map((issue: SecurityIssue) =>
...
Please provide a realistic, actionable remediation roadmap with specific timelines and success criteria.`
  ...
  async answerQuestion(question: string, analysisResults: AnalysisResults): Promise<string> {
    ...
    const contextPrompt = {
      ...
      Sample Issues:
      ${analysisResults.issues.slice(0, 5).map((issue: SecurityIssue) => `- ${issue.severity} ${issue.type}: ${issue.message} in ${issue.filename}:${issue.line}`).join('\n')}
