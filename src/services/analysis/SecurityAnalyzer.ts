...
export class SecurityAnalyzer {
  ...
  private getSecretRecommendation(type: SecretType): string {
    const recommendations: Partial<Record<SecretType, string>> = {
      ...
      'aws_secret_key': 'Remove AWS secret keys from code and use IAM roles or environment variables',
      ...
      'google_oauth': 'Store Google OAuth credentials in secure configuration, not source code',
      ...
      'azure_key': 'Use Azure Key Vault or environment variables instead of hardcoded keys',
      'npm_token': 'Use .npmrc with environment variables instead of hardcoded tokens',
      'pypi_token': 'Use environment variables for PyPI authentication',
      'docker_auth': 'Use Docker credential helpers instead of hardcoded authentication',
      'openai_key': 'Store OpenAI API keys in environment variables or secret managers',
      'anthropic_key': 'Store Anthropic API keys in environment variables or secret managers',
      'firebase_key': 'Use Firebase environment configuration instead of hardcoded keys',
      ...
      'vercel_token': 'Use Vercel environment variables instead of hardcoded tokens',
      'netlify_token': 'Use Netlify environment variables instead of hardcoded tokens',
  ...
  private getSecretImpact(type: SecretType): string {
    const impacts: Partial<Record<SecretType, string>> = {
      ...
      'aws_secret_key': 'Unauthorized access to AWS resources and potential infrastructure compromise',
      'github_token': 'Unauthorized access to repositories and potential code theft',
      'gitlab_token': 'Unauthorized access to repositories and potential code theft',
      'slack_token': 'Unauthorized access to Slack workspace and sensitive communications',
      'slack_webhook': 'Unauthorized posting to Slack channels and potential phishing',
      ...
      'docker_auth': 'Unauthorized access to container registries and image manipulation',
      ...
      'vercel_token': 'Unauthorized deployment access and infrastructure compromise',
      'netlify_token': 'Unauthorized deployment access and infrastructure compromise',
