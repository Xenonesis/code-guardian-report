...
export class DataFlowAnalyzer {
      ...
      if (this.isJavaScriptFile(file.filename)) {
        try {
          ...
          if (ast) {
            ...
            const flowIssues = this.convertFlowsToIssues(
              file.filename,
              file.content
            );
  ...
  private isTaintedExpression(
    ...
    if (t.isCallExpression(node)) {
      const calleeName = t.isExpression(node.callee)
        ...
        : "";
      return Object.values(this.taintSources)
        .flat()
        .some((source) => calleeName.includes(source));
    }
    ...
    if (t.isMemberExpression(node)) {
      ...
      return Object.values(this.taintSources)
        .flat()
        .some((source) => memberName.includes(source));
    }
  ...
  private getSourceType(
    node:
      ...
      | t.ArgumentPlaceholder,
    _content: string
  ): TaintFlow["source"]["type"] {
    if (!("type" in node)) return "external";

    if (t.isCallExpression(node) || t.isMemberExpression(node)) {
      const name =
        t.isCallExpression(node) && t.isExpression(node.callee)
          ? this.getMemberExpressionName(node.callee)
          : t.isMemberExpression(node)
            ? this.getMemberExpressionName(node)
            : "";

      if (this.taintSources.user_input.some((src) => name.includes(src)))
        return "user_input";
      if (this.taintSources.file_read.some((src) => name.includes(src)))
        return "file_read";
      if (this.taintSources.database.some((src) => name.includes(src)))
        return "database";
    }
  ...
  private calculateConfidence(sourceType: string, sinkType: string): number {
    ...
    if (sourceType === "user_input") {
      if (sinkType === "sql" || sinkType === "command" || sinkType === "eval")
        return 95;
  ...
  private getRecommendation(sinkType: string): string {
    const recommendations: Record<string, string> = {
      ...
      file_write:
        "Validate file paths and use path.join() with basedir restriction",
    };
