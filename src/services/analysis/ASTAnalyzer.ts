...
export class ASTAnalyzer {
      ...
      traverseFunction(ast as any, {
        ...
        CallExpression: (path: NodePath<t.CallExpression>) => {
          try {
            ...
            if (
              ...
              this.dangerousFunctions.eval.includes(node.callee.name)
            ) {
              issues.push(
                this.createIssue(
            ...
            if (t.isMemberExpression(node.callee)) {
              ...
              if (
                this.dangerousFunctions.exec.some((fn) =>
                  memberName.includes(fn)
                )
              ) {
                issues.push(
                  this.createIssue(
            ...
            logger.warn(
              `Error in CallExpression visitor for ${filename}:`,
              error
            );
          }
        },
        ...
        JSXAttribute: (path: NodePath<t.JSXAttribute>) => {
          try {
            ...
            logger.warn(
              `Error in JSXAttribute visitor for ${filename}:`,
              error
            );
          }
        },
        ...
        AssignmentExpression: (path: NodePath<t.AssignmentExpression>) => {
          try {
            ...
            if (t.isMemberExpression(node.left)) {
              ...
              if (
                this.dangerousFunctions.xss.some((prop) =>
                  memberName.includes(prop)
                )
              ) {
                issues.push(
                  this.createIssue(
            ...
            logger.warn(
              `Error in AssignmentExpression visitor for ${filename}:`,
              error
            );
          }
        },
        ...
        VariableDeclarator: (path: NodePath<t.VariableDeclarator>) => {
          try {
            ...
            logger.warn(
              `Error in VariableDeclarator visitor for ${filename}:`,
              error
            );
          }
        },
        ...
        MemberExpression: (path: NodePath<t.MemberExpression>) => {
          try {
            ...
            logger.warn(
              `Error in MemberExpression visitor for ${filename}:`,
              error
            );
          }
        },
  ...
  private trackTaintSources(path: NodePath<t.CallExpression>): void {
    try {
      ...
      const sourceName = t.isExpression(node.callee)
  ...
  private trackTaintSinks(path: NodePath<t.CallExpression>): void {
    try {
      const node = path.node;
      const calleeName = t.isExpression(node.callee)
        ? this.getMemberExpressionName(node.callee)
        : "";
  ...
  private getMemberExpressionName(node: t.Expression): string {
    try {
      ...
      if (t.isMemberExpression(node)) {
        const object = this.getMemberExpressionName(
  ...
  private getRecommendation(type: string): string {
    const recommendations: Record<string, string> = {
      "Code Injection via eval":
        "Avoid using eval(). Use JSON.parse() for data or safer alternatives",
      "Command Injection Risk":
        "Validate and sanitize all inputs before command execution. Use parameterized commands",
      "XSS via dangerouslySetInnerHTML":
        "Sanitize HTML content using DOMPurify before rendering",
      "XSS via DOM Manipulation":
        "Use safe DOM manipulation methods or sanitize content before insertion",
      "Hardcoded Secret":
  ...
  private getImpact(type: string): string {
    const impacts: Record<string, string> = {
      ...
      "Command Injection Risk":
        "System command execution, potential server takeover",
      "XSS via dangerouslySetInnerHTML":
        "Cross-site scripting, session hijacking, data theft",
      "XSS via DOM Manipulation":
  ...
  private getTaintFlowRecommendation(sinkType: string): string {
    const recommendations: Record<string, string> = {
      ...
      command:
        "Validate and sanitize input before command execution. Use safe APIs",
