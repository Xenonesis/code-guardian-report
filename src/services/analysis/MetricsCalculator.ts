...
export class MetricsCalculator {
  public calculateQualityScore(
    ...
    const baseDensity =
      linesAnalyzed > 0 ? (totalWeightedIssues / linesAnalyzed) * 1000 : 0;
    ...
    qualityScore = this.applyQualityModifiers(
      qualityScore,
      issues,
      linesAnalyzed
    );
  ...
  private calculateIssueDiversityPenalty(issues: SecurityIssue[]): number {
    ...
    const diversityPenalty =
      uniqueTypes.size > 5 ? (uniqueTypes.size - 5) * 1.5 : 0;
  ...
  private applyQualityModifiers(
    ...
    const hasOnlyLowSeverity = issues.every(
    ...
    if (secretIssues.length > 0) {
      const criticalSecrets = secretIssues.filter(
        (s) => s.severity === "Critical"
      ).length;
      const highSecrets = secretIssues.filter(
        (s) => s.severity === "High"
      ).length;

      // Heavy penalty for exposed secrets
      modifiedScore -=
        criticalSecrets * 8 + highSecrets * 5 + secretIssues.length * 2;
    }

    return Math.max(0, Math.min(100, modifiedScore));
  }

  public calculateVulnerabilityDensity(
    issues: SecurityIssue[],
    linesAnalyzed: number
  ): number {
  ...
  public calculateSummaryMetrics(
    issues: SecurityIssue[],
    linesAnalyzed: number
  ) {
    const criticalIssues = issues.filter(
  ...
  public calculateDetailedMetrics(
    issues: SecurityIssue[],
    linesAnalyzed: number
  ) {
    const criticalIssues = issues.filter(
    ...
    const totalDebtHours =
      criticalDebt + highDebt + mediumDebt + lowDebt + complexityDebt;
  ...
  private calculateMaintainabilityIndex(
    ...
    const issuesDensity =
      linesAnalyzed > 0 ? (issues.length / linesAnalyzed) * 1000 : 0;
    ...
    return Math.max(
      0,
      Math.min(100, baseMaintainability - severityPenalty + distributionBonus)
    );
  }
  ...
  private calculateSeverityImpact(issues: SecurityIssue[]): number {
    ...
    const highRatio =
      issues.filter((i) => i.severity === "High").length / total;
  ...
  private estimateDuplicatedLines(
    issues: SecurityIssue[],
    linesAnalyzed: number
  ): number {
  ...
  private calculateTestCoverage(
    ...
    const testRelatedIssues = issues.filter(
      (issue) =>
    ...
    const issueRatio =
      issues.length > 0 ? (issues.length / linesAnalyzed) * 1000 : 0;

    let baseCoverage = hasTestFiles ? 75 : 35;

    // Adjust based on code quality indicators
    if (issueRatio < 1)
      baseCoverage += 15; // Low issue density suggests good testing
  ...
  private calculateCodeComplexity(
    ...
    const complexityIndicators = issues.filter(
      (issue) =>
    ...
    const baseComplexity =
      linesAnalyzed > 0 ? (complexityIndicators / linesAnalyzed) * 10000 : 0;
  ...
  private calculateSecurityMaturity(issues: SecurityIssue[]): number {
    ...
    const maturityScore =
      100 - criticalRatio * 60 - (totalIssues / 10) * 5 - secretPenalty;
  ...
  private calculatePerformanceScore(issues: SecurityIssue[]): number {
    const performanceRelatedIssues = issues.filter(
      (issue) =>
    ...
    const performanceScore =
      100 - performanceRatio * 50 - performanceRelatedIssues * 5;
