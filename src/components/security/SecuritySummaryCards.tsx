...
export const SecuritySummaryCards: React.FC<SecuritySummaryCardsProps> = ({ results }) => {
  ...
  const getScoreBgColor = (score: number) => {
    ...
    if (score >= 40) return 'from-orange-50 to-red-50 dark:from-orange-950/20 dark:to-red-950/20 border-orange-200 dark:border-orange-800';
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-4 md:gap-6">
        <Tooltip>
          <TooltipTrigger asChild>
            <Card className={`bg-gradient-to-br ${getScoreBgColor(results.summary.securityScore)} hover:shadow-md transition-shadow duration-200 cursor-help`}>
              <CardContent className="p-4 sm:p-4">
                <div className="flex items-center gap-3">
                  <div className={`p-2 bg-gradient-to-br ${getScoreColor(results.summary.securityScore)} rounded-lg flex-shrink-0`}>
                    <Shield className="h-5 w-5 text-white" />
                  </div>
                  <div className="min-w-0">
                    <p className={`text-2xl font-bold ${getScoreTextColor(results.summary.securityScore)} truncate`}>
                      {results.summary.securityScore}
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-400 truncate">Security Score</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <TooltipContent className="max-w-xs">
            ...
            <p className="text-xs mt-1">Calculated based on severity and density of security issues.</p>
          </TooltipContent>
        </Tooltip>
        ...
        <Tooltip>
          ...
          <TooltipContent className="max-w-xs">
            ...
            <p className="text-xs mt-1">Critical: {results.summary.criticalIssues} | High: {results.summary.highIssues}</p>
          </TooltipContent>
        </Tooltip>
        ...
        <Tooltip>
          ...
          <TooltipContent className="max-w-xs">
            ...
            <p className="text-xs mt-1">Vulnerabilities per 1,000 lines of code. Lower is better.</p>
          </TooltipContent>
        </Tooltip>
        ...
        <Tooltip>
          ...
          <TooltipContent className="max-w-xs">
            ...
            <p className="text-xs mt-1">API keys, passwords, and sensitive data found in code.</p>
          </TooltipContent>
        </Tooltip>

        <Tooltip>
          <TooltipTrigger asChild>
            <Card className={`bg-gradient-to-br ${getScoreBgColor(results.summary.qualityScore)} hover:shadow-md transition-shadow duration-200 cursor-help`}>
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  <div className={`p-2 bg-gradient-to-br ${getScoreColor(results.summary.qualityScore)} rounded-lg`}>
                    <TrendingUp className="h-5 w-5 text-white" />
                  </div>
                  <div>
                    <p className={`text-2xl font-bold ${getScoreTextColor(results.summary.qualityScore)}`}>
          ...
          <TooltipContent className="max-w-xs">
            ...
            <p className="text-xs mt-1">Overall quality based on maintainability and best practices.</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </TooltipProvider>
