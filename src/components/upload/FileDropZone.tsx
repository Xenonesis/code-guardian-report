...
export const FileDropZone: React.FC<FileDropZoneProps> = ({ onFileSelect }) => {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-gradient-to-r from-blue-400/10 via-purple-400/10 to-indigo-400/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
        aria-hidden="true"
      ></div>
      <div className="relative z-10">
        <div className="mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300">
          <Upload
            className="h-12 w-12 sm:h-16 sm:w-16 mx-auto text-slate-400 group-hover:text-blue-500 transition-colors duration-300"
            aria-hidden="true"
          />
        </div>
