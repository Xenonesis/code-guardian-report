...
describe('GitHub Analysis Dashboard - Phase 4 Features', () => {
  ...
  describe('Repository Comparison Tool', () => {
    ...
    it('should identify best performing repository', async () => {
      ...
      if (repos.length >= 2) {
        const bestRepo = repos.reduce((best, repo) => 
    ...
    it('should compare issue counts across repositories', async () => {
      ...
      if (repos.length >= 2) {
        const totalIssues = repos.reduce((sum, repo) => sum + repo.issuesFound, 0);
  ...
  describe('Code Quality Analytics', () => {
    it('should calculate complexity scores based on security data', async () => {
      ...
      repos.forEach(repo => {
        ...
        const complexityValue = Math.max(1, Math.min(10, 10 - repo.securityScore + (repo.issuesFound / 10)));
    ...
    it('should calculate maintainability index correctly', async () => {
      ...
      repos.forEach(repo => {
        ...
        const index = Math.max(0, Math.min(100, baseValue - (repo.criticalIssues * penaltyPerCritical)));
    ...
    it('should calculate technical debt correctly', async () => {
      ...
      repos.forEach(repo => {
        ...
        const formatted = days === 0 ? `${totalHours}h` : `${days}d ${totalHours % 8}h`;
    ...
    it('should aggregate quality metrics across repositories', async () => {
      ...
      if (repos.length > 0) {
        ...
        const complexityScores = repos.map(repo => 
          Math.max(1, Math.min(10, 10 - repo.securityScore + (repo.issuesFound / 10)))
        );
        const avgComplexity = complexityScores.reduce((sum, s) => sum + s, 0) / repos.length;
        ...
        const maintainabilityScores = repos.map(repo => {
          const baseValue = repo.securityScore * 10;
          return Math.max(0, Math.min(100, baseValue - (repo.criticalIssues * 5)));
        });
        const avgMaintainability = maintainabilityScores.reduce((sum, s) => sum + s, 0) / repos.length;
    ...
    it('should calculate code duplication estimates', () => {
      ...
      testCases.forEach(({ issues, expected }) => {
        const duplication = Math.min(30, Math.round(issues * 0.5 + Math.random() * 5));
  ...
  describe('Vulnerability Pattern Analytics', () => {
    ...
    it('should analyze vulnerabilities by language', async () => {
      ...
      const languageMap = new Map<string, { total: number; critical: number }>();
    ...
    it('should identify trending vulnerabilities', async () => {
      ...
      const avgIssues = repos.reduce((sum, r) => sum + r.issuesFound, 0) / repos.length;
    ...
    it('should calculate vulnerability heatmap data', async () => {
      ...
      if (repos.length > 0) {
        // Group by language
        const heatmap = repos.reduce((map, repo) => {
          const lang = repo.language || 'Unknown';
          if (!map[lang]) {
  ...
  describe('Integration Tests - All Features', () => {
    ...
    it('should calculate aggregate metrics correctly', async () => {
      ...
      if (repos.length > 0) {
        ...
        const totalIssues = repos.reduce((sum, r) => sum + r.issuesFound, 0);
        const avgScore = repos.reduce((sum, r) => sum + r.securityScore, 0) / totalRepos;
