...
export const useAnalyticsData = (
  ...
  const fileComplexityData = useMemo(() => {
    ...
    return Object.values(fileStats)
      .map((item) => ({
        ...
        riskScore:
          item.severities.Critical * 4 +
          item.severities.High * 3 +
          item.severities.Medium * 2 +
          item.severities.Low * 1,
      }))
  ...
  const riskMetrics = useMemo(() => {
    ...
    return {
      ...
      codeQuality: Math.max(
        0,
        100 - issues.filter((i) => i.type === "Code Smell").length * 5
      ),
      maintainability: Math.max(
        0,
        100 - Math.round((issues.length / totalFiles) * 10)
      ),
      technicalDebt: Math.min(
        100,
        Math.round((issues.length / totalFiles) * 15)
      ),
    };
  }, [issues, totalFiles]);
  ...
  const performanceData = useMemo(
    () => [
      ...
      {
        metric: "Security",
        value: Math.max(0, 100 - riskMetrics.securityIssues * 10),
        target: 95,
      },
      {
        metric: "Maintainability",
        value: riskMetrics.maintainability,
        target: 80,
      },
      {
        metric: "Technical Debt",
        value: Math.max(0, 100 - riskMetrics.technicalDebt),
        target: 70,
      },
    ],
    [riskMetrics]
