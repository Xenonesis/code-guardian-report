...
import { NotificationManager, notify } from '@/services/notifications/NotificationManager';
import type { NotificationCategory, NotificationPriority } from '@/services/notifications/NotificationManager';
...
export const enhancedNotifications = {
  ...
  dataLoadError: () => {
    notify.error('Failed to Load Data', {
      message: 'Unable to fetch your data. Please check your connection and try again.',
      priority: 'high',
      category: 'storage',
    });
  },
  ...
  criticalIssuesFound: (count: number, filename: string) => {
    notify.warning('Critical Security Issues Detected', {
      ...
      action: {
        label: 'Review Now',
        onClick: () => {
          document.getElementById('security-issues')?.scrollIntoView({ behavior: 'smooth' });
        },
      },
    });
  },
  ...
  signInSuccess: (username?: string) => {
    notify.success('Signed In', {
      message: username ? `Welcome back, ${username}!` : 'Successfully signed in.',
      priority: 'normal',
      category: 'auth',
      metadata: { username },
    });
  ...
  historyLoadError: () => {
    notify.warning('History Unavailable', {
      message: 'Unable to load analysis history from cloud. Check your connection.',
      priority: 'normal',
      category: 'network',
    });
  },
  ...
  maintenanceMode: () => {
    notify.warning('Maintenance Mode', {
      message: 'The application is in maintenance mode. Some features may be unavailable.',
      priority: 'high',
      category: 'system',
    });
  },
...
export const batchNotifications = {
   ...
   */
  analysisResults: (results: Array<{ filename: string; issueCount: number }>) => {
